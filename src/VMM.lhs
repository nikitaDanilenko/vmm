...
function. It is very similar to `zipWith`. The `L` at the end hints at the result type being a list.
...
The more homogeneous version of the intersection function can be implemented with the above function
as follows.
...
Another useful application is to compute the vector that represents all vertices that are
reachable from a given start vector.
To that end we need to combine the vectors in the list produced by `reachWith`.
Since this list contains only distinct reachability layers,
all of its elements are pairwise disjoint.
Thus we can use `leftmostUnion` to merge the results without losing any information.
...
A simplified version of `disjointPaths` where the values in the start vector are ignored and
replaced by the empty forest.
